...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    cal << %(<th colspan="#{colspan + 1}" class="#{options[:month_name_class]}">#{Date::MONTHNAMES[options[:month]]}</th>)
    cal << %(<th colspan="2">#{options[:next_month_text]}</th>) if options[:next_month_text]
    cal << %(</tr><tr class="#{options[:day_name_class]}">)
    day_names.each_with_index do |d, index|
      unless d[options[:abbrev]].eql? d
        cal << "<th scope='col' class='#{'weekendDayHeader' if index == 0 || index == 6}'><abbr title='#{d}'>#{d[options[:abbrev]]}</abbr></th>"
      else
        cal << "<th scope='col' class='#{'weekendDayHeader' if index == 0 || index == 6}'>#{d[options[:abbrev]]}</th>"
      end
    end
    cal << "<th class='total_sum_header'>Total</th>"
    ...
    first.upto(last) do |cur|
      ...
      cal << "<td class='total_sum'></td></tr><tr>" if cur.wday == last_weekday
    end
    ...
    cal << "<td class='total_sum'></td></tr></tbody></table>"
    cal.join("\n")
  end
